...
# Ensure files are ordered according to metatdata sample sheet
...
# converting myobj to tile object for sliding 100 bp windows minimum coverage of 5 CpGs 
...
datatable(
  ...
  caption= htmltools::tags$caption(
    ...
    "3X Tile = Number of 100 bp tiles each with at least 5 CpGs each with at least 3X coverage."
  )
) %>% formatRound(
  columns = "sample_tile_cnts",
  digits = 0
)
```

## Coverage Statistics
Tiles must meet coverage requirements in all samples to be included in analysis.

```{r coverage_stats, results="asis"}

cat("Tiles with at least 3X coverage in all samples:") 
...
perc.meth.tile.sig <- perc.meth.tile[perc.meth.tile$qvalue < 0.05 & abs(perc.meth.tile$meth.diff) > 10,]

write.table(
  perc.meth.tile.sig,
  file = paste0(params$out_path, "/", params$study,"_dmr_qval_0.05_methdiff_10.tsv"),
  sep="\t",
  col.names=TRUE,
  row.names=FALSE,
  quote=FALSE
...
## Tile % methylation scatterplot

All `r nrow(perc.meth.tile)` Tiles are plotted with 
average % methylation in `r paste0(contrast[1], " vs ", contrast[2])`.

```{r tile_scatterplot, fig.height=10, fig.width=12, dev="png"}
...
perc.meth.tile$density <- get_density(
  perc.meth.tile$avg_ctrl, perc.meth.tile$avg_treat, n = 100)
...
ggplot(perc.meth.tile) + 
  ...
  labs(
    ...
    title=paste(contrast[1], "vs", contrast[2], "Tiles")
  ) +
  th 
```
...
**`r total_sig_dmrs`** DMRs were identified with q-value < 0.05 and % methylation difference > 10.
...
perc.meth.tile$sig.group[perc.meth.tile$qvalue < 0.05 & abs(perc.meth.tile$meth.diff) <= 10] <- "qvalue"
perc.meth.tile$sig.group[perc.meth.tile$qvalue >= 0.05 & abs(perc.meth.tile$meth.diff) > 10] <- "meth.diff"
perc.meth.tile$sig.group[perc.meth.tile$qvalue < 0.05 & abs(perc.meth.tile$meth.diff) > 10] <- "qvalue and meth.diff"
