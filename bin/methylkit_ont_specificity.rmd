...
rmarkdown::render(
  ...
  params = list(
    ...
    modkit_path = "/home/tylerborrman/epic-methylseq_data/test_12ABIC",
...
params:
  ...
  modkit_path: path to directory containing bismark coverage files (.deduplicated.bismark.cov)
...
} else if ("KAL_silenced" %in% metadata$condition & "CRISPRon" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "CRISPRon", 1, 0)
  treated <- metadata$sample[metadata$condition == "CRISPRon"]
  control <- metadata$sample[metadata$condition == "KAL_silenced"]
  contrast <- c("CRISPRon", "KAL_silenced")
} else if ("TKTL1_target" %in% metadata$condition & "Non_target" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "TKTL1_target", 1, 0)
  treated <- metadata$sample[metadata$condition == "TKTL1_target"]
  control <- metadata$sample[metadata$condition == "Non_target"]
  contrast <- c("TKTL1_target", "Non_target")
} else if ("BRDT_target" %in% metadata$condition & "Non_target" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "BRDT_target", 1, 0)
  treated <- metadata$sample[metadata$condition == "BRDT_target"]
  control <- metadata$sample[metadata$condition == "Non_target"]
  contrast <- c("BRDT_target", "Non_target")
} else if ("HSPA1A_target" %in% metadata$condition & "Non_target" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "HSPA1A_target", 1, 0)
  treated <- metadata$sample[metadata$condition == "HSPA1A_target"]
  control <- metadata$sample[metadata$condition == "Non_target"]
  contrast <- c("HSPA1A_target", "Non_target")
} else {
...
datatable(
  ...
  caption= htmltools::tags$caption(
    ...
    "10X CpG = Number of CpGs in on and off-target regions with at least 10X coverage."
  )
) %>% formatRound(
...
cat("On and off-target region CpGs with at least 10X coverage in all samples:") 
