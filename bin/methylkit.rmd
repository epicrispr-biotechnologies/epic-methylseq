...
myobj=invisible(methRead(cov_files,
...
meth=invisible(methylKit::unite(myobj, destrand=FALSE))
```
...
invisible(capture.output({tile.obj = tileMethylCounts(myobj,win.size=100,step.size=100,cov.bases=5)}))
...
meth.tile=invisible(methylKit::unite(tile.obj, destrand=FALSE))
```
...
```{r sample_table}
sample_cpg_cnts <- sapply(myobj, function(x) x@num.records)
sample_tile_cnts <- sapply(tile.obj, function(x) x@num.records)
...
datatable(
  ...
  caption= htmltools::tags$caption(
    ...
    "10X Tile = Number of tiles with at least 5 CpGs each with at least 10X coverage.", htmltools::tags$br(), 
    "10X Tile % = Percentage of tiles with at least 5 CpGs each with at least 10X coverage of total tiles."
  )
) %>% formatStyle(
  columns="percent_cpg",
...
unite_percent_cpg <- round((meth@num.records / genome_cpg_cnt) * 100, 2)
unite_percent_tile <- round((meth.tile@num.records / genome_tile_cnt) * 100, 2)

if (unite_percent_cpg >= 75) {
  cat("CpGs with at least 10X coverage in all samples:") 
  cat(format(meth@num.records, big.mark = ","))
  cat("\n\nPercentage of CpGs with at least 10X coverage in all samples of total CpGs:    ")
  cat(paste0(unite_percent_cpg, "%"))
} else {
  cat("CpGs with at least 10X coverage in all samples: ")
  cat(paste0("<span style='color: red;'>", format(meth@num.records, big.mark = ",") , "</span>"))
  cat("\n\nPercentage of CpGs with at least 10X coverage in all samples of total CpGs:    ")
  cat(paste0("<span style='color: red;'>", unite_percent_cpg , "%</span>"))
}
if (unite_percent_tile >= 75) {
  cat("\n\nTiles with at least 5 CpGs each with at least 10X coverage in all samples:    ") 
  cat(format(meth.tile@num.records, big.mark = ","))
  cat("\n\nPercentage of tiles with at least 5 CpGs each with at least 10X coverage in all samples of total tiles:    ")
  cat(paste0(unite_percent_tile, "%"))
} else {
  cat("\n\nTiles with at least 5 CpGs each with at least 10X coverage in all samples:    ") 
  cat(paste0("<span style='color: red;'>", format(meth.tile@num.records, big.mark = ",") , "</span>"))
  cat("\n\nPercentage of tiles with at least 5 CpGs each with at least 10X coverage in all samples of total tiles:    ")
...
meth.cpg.mat <- myPCASamples(meth)
```
...
# Plot PCA biplot
meth.tile.mat <- myPCASamples(meth.tile)
```
...
Differentially methylated bases or regions are calcualted using methylKit.
...
# write bedfile
meth.dmc.bed <- as.data.frame(meth.dmc)
meth.dmr.bed <- as.data.frame(meth.dmr)
write.table(meth.dmc.bed, paste0(params$out_path, "/", params$study,".meth.cpg.all.bed"), sep="\t", col.names=FALSE, row.names=FALSE, quote=FALSE)
write.table(meth.dmr.bed, paste0(params$out_path, "/", params$study,".meth.tile.all.bed"), sep="\t", col.names=FALSE, row.names=FALSE, quote=FALSE)
```
...
meth.dmc.bed$coord <- paste0(meth.dmc.bed$chr,".",meth.dmc.bed$start,".",meth.dmc.bed$end)
perc.meth.cpg <- merge(perc.meth.cpg, meth.dmc.bed, by="coord")
...
meth.dmr.bed$coord <- paste0(meth.dmr.bed$chr,".",meth.dmr.bed$start,".",meth.dmr.bed$end)
perc.meth.tile <- merge(perc.meth.tile, meth.dmr.bed, by="coord")
...
ggplot(perc.meth.cpg) + 
  ...
  geom_abline(intercept=-10, slope=1, linetype="dashed", color="red") +
  labs(
    x="Average % methylation in CONTROL",
    y="Average % methylation in EPI321",
    title="EPI321 vs CONTROL CpGs"
  ) +
  th 
```
...
```{r chromhmm_plot, fig.height=10, fig.width=13}
...
```{r ccre_plot, fig.height=10, fig.width=13}
