...
chromhmm_df <- chromhmm_df[c("seqnames", "start", "end", "name")]
colnames(chromhmm_df) <- c("ChromHMM.chr", "ChromHMM.start", "ChromHMM.end", "ChromHMM.state")

# Generate Granges object from DMRs dataframe
dmrs_df <- perc.meth.tile.sig[c("chr", "start", "end", "qvalue", "meth.diff")]
colnames(dmrs_df) <- c("DMR.chr", "DMR.start", "DMR.end", "DMR.qvalue", "DMR.meth.diff")
dmrs_gr <- makeGRangesFromDataFrame(
  dmrs_df,
  ignore.strand = TRUE,
  keep.extra.columns = TRUE,
  seqnames.field = "DMR.chr",
...
overlap_df <- cbind(dmrs_overlap, chromhmm_overlap)
...
```{r chromhmm_table}
# Display in table
round_cols <- c("DMR.qvalue", "DMR.meth.diff")
bp_cols <- c("DMR.start", "DMR.end", "ChromHMM.start", "ChromHMM.end")
