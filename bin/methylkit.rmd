...
# Add prefix "chr" to contig names in gtf for nearest gene searching.
# For some of the contigs like KI270711.1 it does not make sense to prefix with "chr".
# However, since we are only interested in canonical chromosomes, 
# adding the prefix to all contigs should be fine. 
seqlevels(tss_gr) <- paste0("chr", seqlevels(tss_gr))
...
nearest_genes_df <- cbind(dmrs_df, nearest_genes, distances)
```

```{r nearest_gene_plot, fig.height=10, fig.width=13}
ggplot(nearest_genes_df, aes(x=distance)) +
  geom_histogram(fill="#1c3f74", bins=50) + 
