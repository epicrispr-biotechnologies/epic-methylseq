...
if ("EPI321" %in% metadata$condition & "CONTROL" %in% metadata$condition) {
  ...
  control <- metadata$sample[metadata$condition == "NT"]
  contrast <- c("STOP", "NT")
} else if ("Myo_D7" %in% metadata$condition & "Testes" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "Myo_D7", 1, 0)
  treated <- metadata$sample[metadata$condition == "Myo_D7"]
  control <- metadata$sample[metadata$condition == "Testes"]
  contrast <- c("Myo_D7", "Testes")
} else {
...
perc.meth.tile.sig <- perc.meth.tile[perc.meth.tile$qvalue < 0.05 & abs(perc.meth.tile$meth.diff) > 5,]
total_sig_dmrs <- nrow(perc.meth.tile.sig)
run_dmr_annotation <- total_sig_dmrs > 0

write.table(
  perc.meth.tile.sig,
  file = paste0(params$out_path, "/", params$study,"_dmr_q0.05_methdiff_5.tsv"),
  sep="\t",
  col.names=TRUE,
  row.names=FALSE,
  quote=FALSE
...
**`r total_sig_dmrs`** DMRs were identified with q-value < 0.05 and % methylation difference > 5.
...
perc.meth.tile$sig.group[perc.meth.tile$qvalue < 0.05 & abs(perc.meth.tile$meth.diff) <= 5] <- "qvalue"
perc.meth.tile$sig.group[perc.meth.tile$qvalue >= 0.05 & abs(perc.meth.tile$meth.diff) > 5] <- "meth.diff"
perc.meth.tile$sig.group[perc.meth.tile$qvalue < 0.05 & abs(perc.meth.tile$meth.diff) > 5] <- "qvalue and meth.diff"
...
ggplot(
  ...
  labs(
    ...
    subtitle = "Thresholds: qvalue=0.05, meth.diff=5"
  ) +
  guides(
    color = guide_legend(
      override.aes = list(
...
```{r nearest_gene, eval=run_dmr_annotation, results="asis"}
...
cat(paste0("<b>", count_dmr_overlap, " / ", total_sig_dmrs, " (", "</b>"))
cat(paste0("<b>", round((count_dmr_overlap / total_sig_dmrs) * 100, 2), "%)", "</b>"))
cat(" of DMRs have a nearest gene annotation.")
...
```{r nearest_gene_table, eval=FALSE}
# Display in table
...
```{r chromhmm, eval=run_dmr_annotation, results = "asis"}
...
if (basename(params$chromhmm_path) == "hg38_genome_100_segments.bed") {
  cat("ChromHMM annotations were sourced from ")
  cat('<a href="https://github.com/ernstlab/full_stack_ChromHMM_annotations">https://github.com/ernstlab/full_stack_ChromHMM_annotations</a>')
}

if (basename(params$chromhmm_path) == "mm10_100_segments_segments.bed") {
  cat("ChromHMM annotations were sourced from ")
  cat('<a href="https://github.com/ernstlab/mouse_fullStack_annotations">https://github.com/ernstlab/mouse_fullStack_annotations</a>')
}
cat("\n\n")
cat(paste0("<b>", count_dmr_overlap, " / ", total_sig_dmrs, " (", "</b>"))
cat(paste0("<b>", round((count_dmr_overlap / total_sig_dmrs) * 100, 2), "%)", "</b>"))
cat(" of DMRs overlap with ChromHMM annotations")
cat("\n\n")
cat("Note: a single DMR may have multiple ChromHMM annotations.")
```
...
```{r ccre, eval=run_dmr_annotation, results = "asis"}
...
if (basename(params$ccre_path) == "GRCh38-cCREs.bed") {
  cat("cCRE (candidate cis-regulatory element) annotations were sourced from ")
  cat('<a href="https://screen.encodeproject.org/">https://screen.encodeproject.org/</a>')
  cat(" all human cCREs (hg38) Registry of cCREs V3")
}
if (basename(params$ccre_path) == "mm10-cCREs.bed") {
  cat("cCRE (candidate cis-regulatory element) annotations were sourced from ")
  cat('<a href="https://screen.encodeproject.org/">https://screen.encodeproject.org/</a>')
  cat(" all mouse cCREs (mm10) Registry of cCREs V3")
}
cat("\n\n")
cat(paste0("<b>", count_dmr_overlap, " / ", total_sig_dmrs, " (", "</b>"))
cat(paste0("<b>", round((count_dmr_overlap / total_sig_dmrs) * 100, 2), "%)", "</b>"))
cat(" of DMRs overlap with cCRE annotations")
cat("\n\n")
cat("Note: a single DMR may have multiple cCRE annotations.")
```
...
```{r blacklist, eval=run_dmr_annotation, results = "asis"}
...
if (basename(params$blacklist_path) == "hg38-blacklist.v2.bed") {
  cat("blacklist annotations were sourced from ")
  cat('<a href="https://github.com/Boyle-Lab/Blacklist/blob/master/lists/hg38-blacklist.v2.bed.gz">https://github.com/Boyle-Lab/Blacklist/blob/master/lists/hg38-blacklist.v2.bed.gz</a>')
}
if (basename(params$blacklist_path) == "mm10-blacklist.v2.bed") {
  cat("blacklist annotations were sourced from ")
  cat('<a href="https://github.com/Boyle-Lab/Blacklist/blob/master/lists/mm10-blacklist.v2.bed.gz">https://github.com/Boyle-Lab/Blacklist/blob/master/lists/mm10-blacklist.v2.bed.gz</a>')
}
cat("\n\n")
cat(paste0("<b>", count_dmr_overlap, " / ", total_sig_dmrs, " (", "</b>"))
cat(paste0("<b>", round((count_dmr_overlap / total_sig_dmrs) * 100, 2), "%)", "</b>"))
cat(" of DMRs overlap with blacklist annotations")
cat("\n\n")
cat("Note: a single DMR may have multiple blacklist annotations.")

```

```{r blacklist_table, eval=FALSE}
if (count_dmr_overlap > 0) {
  # Display in table
  round_cols <- c("qvalue", "meth.diff")
  bp_cols <- c("start", "end")
