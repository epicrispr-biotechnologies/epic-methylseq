...
scores_to_rgb <- function(scores) {
  ...
  if (diff(limits) == 0) {
    ...
    normalized[finite] <-
      (scores[finite] - limits[1]) / diff(limits)
  }
...
methdiff_rgb <- scores_to_rgb(perc.meth.cpg$meth.diff)
# Make color key png
png(paste0(params$out_path, "/", "meth_diff_color_key.png"), width = 1200, height=400, res = 150)
  ...
  palette <- colorRampPalette(c("#FF2020", "#BDBDBD", "#0066FF"))(256)
  graphics::image(
    x = seq(limits[1], limits[2], length.out = 256),
    y = 1,
