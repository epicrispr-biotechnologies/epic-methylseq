...
rmarkdown::render(
  ...
  output_file = "JIRA_BDS-XXXX_12ABIC_EPI321_vs_CONTROL.html"
)
...
if ("EPI321" %in% metadata$condition & "CONTROL" %in% metadata$condition) {
  ...
  contrast <- c("D7", "D2")
} else if ("EPI321" %in% metadata$condition & "UNTREATED" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "EPI321", 1, 0)
  treated <- metadata$sample[metadata$condition == "EPI321"]
  control <- metadata$sample[metadata$condition == "UNTREATED"]
  contrast <- c("EPI321", "UNTREATED")
} else if ("EPI321" %in% metadata$condition & "STOP" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "EPI321", 1, 0)
  treated <- metadata$sample[metadata$condition == "EPI321"]
  control <- metadata$sample[metadata$condition == "STOP"]
  contrast <- c("EPI321", "STOP")
} else if ("STOP" %in% metadata$condition & "UNTREATED" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "STOP", 1, 0)
  treated <- metadata$sample[metadata$condition == "STOP"]
  control <- metadata$sample[metadata$condition == "UNTREATED"]
  contrast <- c("STOP", "UNTREATED")
} else if ("EPI321" %in% metadata$condition & "NT" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "EPI321", 1, 0)
  treated <- metadata$sample[metadata$condition == "EPI321"]
  control <- metadata$sample[metadata$condition == "NT"]
  contrast <- c("EPI321", "NT")
} else if ("NT" %in% metadata$condition & "UNTREATED" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "NT", 1, 0)
  treated <- metadata$sample[metadata$condition == "NT"]
  control <- metadata$sample[metadata$condition == "UNTREATED"]
  contrast <- c("NT", "UNTREATED")
} else if ("NT" %in% metadata$condition & "STOP" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "STOP", 1, 0)
  treated <- metadata$sample[metadata$condition == "STOP"]
  control <- metadata$sample[metadata$condition == "NT"]
  contrast <- c("STOP", "NT")
} else if ("KAL_silenced" %in% metadata$condition & "dCas9-miniTETv1" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "dCas9-miniTETv1", 1, 0)
  treated <- metadata$sample[metadata$condition == "dCas9-miniTETv1"]
  control <- metadata$sample[metadata$condition == "KAL_silenced"]
  contrast <- c("dCas9-miniTETv1", "KAL_silenced")
} else if ("TREATED" %in% metadata$condition & "CONTROL" %in% metadata$condition) {
  treatment <- ifelse(metadata$condition == "TREATED", 1, 0)
  treated <- metadata$sample[metadata$condition == "TREATED"]
  control <- metadata$sample[metadata$condition == "CONTROL"]
...
myobj=methRead(as.list(cov_files),
...
## Reference genome
Reference genome: ``r params$assembly``
...
datatable(
  ...
  caption= htmltools::tags$caption(
    ...
    "10X CpG = Number of CpGs in target region with at least 10X coverage in the sample"
  )
) %>% formatRound(
  columns = "sample_cpg_cnts",
  digits = 0
)
```

## Coverage Statistics
CpGs must meet coverage requirements in all samples to be included in analysis.

```{r coverage_stats, results="asis"}
cat("Target region CpGs with at least 10X coverage in all samples:") 
...
myDiff=calculateDiffMeth(meth, mc.cores=cores, adjust = "bonferroni")
...
perc.meth.cpg.sig <- perc.meth.cpg[perc.meth.cpg$qvalue < 0.05 & abs(perc.meth.cpg$meth.diff) > 10,]
write.table(
  perc.meth.cpg.sig,
  file = paste0(params$out_path, "/", params$study,"_dmc_qval_0.05_methdiff_10.tsv"),
  sep="\t",
  col.names=TRUE,
  row.names=FALSE,
  quote=FALSE
)
total_sig_dmcs <- nrow(perc.meth.cpg.sig)
```
...
## DMC table
Differentially methylated CpGs are calculated using methylKit.
...
**`r total_sig_dmcs`** DMCs were identified with q-value < 0.05 and % methylation difference > 10.

```{r dmc_table}
# Display in table
non_sample_cols <- c("coord", "avg_treat", "avg_ctrl", "chr", "start", "end", "strand", "pvalue", "qvalue", "meth.diff")
sample_cols <- colnames(perc.meth.cpg.sig)[!colnames(perc.meth.cpg.sig) %in% non_sample_cols]
dmc_table <- perc.meth.cpg.sig[sample_cols]
dmc_table <- cbind(
  perc.meth.cpg.sig[c("chr", "start", "end")],
  dmc_table,
  perc.meth.cpg.sig[c("avg_treat", "avg_ctrl", "pvalue","qvalue", "meth.diff")]
)
...
datatable(
  ...
  options = list(
    ...
    buttons = list(
      ...
      list(
        extend = "csv",
        filename = paste(params$study, "dmc", sep="_")
      ),
      list(
        extend = "excel",
        filename = paste(params$study, "dmc", sep="_")
  ...
  caption= htmltools::tags$caption(
    style = 'caption-side: bottom;',  
    paste("avg_treat = Average DMC % methylation across", contrast[1], "samples"), htmltools::tags$br(),
    paste("avg_ctrl = Average DMC % methylation across", contrast[2], "samples"), htmltools::tags$br(),
    paste("meth.diff =", contrast[1], "DMC % methylation -", contrast[2], "DMC % methylation"), htmltools::tags$br()
    #paste("Chromosome coordinates are in BED format: O-based inclusive start, exclusive end")
  )
) %>%
formatSignif(
  columns = round_cols,
...
```{r write_beds}
# Function to convert scores to RGB colors using viridis palette
# @param scores Numeric vector of scores to convert to RGB colors.
# @return A matrix of RGB values corresponding to the input scores.
scores_to_rgb <- function(scores, set_limit) {
  scores <- as.numeric(scores)
  finite <- is.finite(scores)

  # Equivalent to matplotlib.colors.Normalize()
  limits <- range(scores[finite])
  if (max(abs(limits)) > set_limit) {
    stop("% methylation difference > 40 for BED color limits. Adjust limits")
  }
  normalized <- rep(NA_real_, length(scores))

  if (diff(limits) == 0) {
    normalized[finite] <- 0
  } else {
    normalized[finite] <-
      (scores[finite] - (-set_limit) ) / (set_limit * 2)
  }
  # Interpolate through viridis and convert hex colors to 0–255 RGB
  # palette <- rev(viridisLite::viridis(256))
  # palette <- colorspace::diverging_hcl(256, palette = "Blue-Red")
  palette <- colorRampPalette(c("#FF2020", "#BDBDBD", "#0066FF"))(256)
  mapper <- grDevices::colorRamp(palette, space = "Lab")
  rgb <- round(mapper(normalized))
  rgb = paste(rgb[,1], rgb[,2], rgb[,3], sep=",")
  return(rgb)
}

# Write output to bedfiles
# Make RGB vector for DMCs based on methylation difference
set_limit <- 40
methdiff_rgb <- scores_to_rgb(perc.meth.cpg$meth.diff, set_limit)
# Make color key png
png(paste0(params$out_path, "/", "meth_diff_color_key.png"), width = 1200, height=400, res = 150)
  limits <- range(perc.meth.cpg$meth.diff, na.rm = TRUE)
  # palette <- rev(viridisLite::viridis(256))
  # palette <- colorspace::diverging_hcl(256, palette = "Blue-Red")
  palette <- colorRampPalette(c("#FF2020", "#BDBDBD", "#0066FF"))(256)
  graphics::image(
    x = seq(-set_limit, set_limit, length.out = 256),
    y = 1,
    z = matrix(seq_len(256), ncol = 1),
    col = palette,
    axes = FALSE,
  ...
  graphics::box()
dev.off()
# Make booleans for significant DMCs
sig_methdiff_10 <- perc.meth.cpg$qvalue < 0.05 & abs(perc.meth.cpg$meth.diff) > 10
sig_methdiff_5 <- perc.meth.cpg$qvalue < 0.05 & abs(perc.meth.cpg$meth.diff) > 5
# Create RGB vector for significant DMCs
sig_methdiff_10_rgb <- ifelse(sig_methdiff_10, "255,0,0", "192,192,192")
sig_methdiff_5_rgb <- ifelse(sig_methdiff_5, "255,0,0", "192,192,192")
# Adjust start coordinate for BED format
bed_start <- perc.meth.cpg$start - 1
# Make BED tables
sig_methdiff_10_bed <- cbind(perc.meth.cpg$chr, bed_start, perc.meth.cpg$end, ".", ".", ".", bed_start, perc.meth.cpg$end, sig_methdiff_10_rgb)
sig_methdiff_5_bed <- cbind(perc.meth.cpg$chr, bed_start, perc.meth.cpg$end, ".", ".", ".", bed_start, perc.meth.cpg$end, sig_methdiff_5_rgb)
meth_diff_rgb_bed <- cbind(perc.meth.cpg$chr, bed_start, perc.meth.cpg$end, ".", ".", ".", bed_start, perc.meth.cpg$end, methdiff_rgb)
# Write BED files
write.table(
  sig_methdiff_10_bed,
  file = paste0(params$out_path, "/", params$study,"_dmc_qval_0.05_methdiff_10.bed"),
  sep="\t",
  col.names=FALSE,
  row.names=FALSE,
  quote=FALSE
...
write.table(
  sig_methdiff_5_bed,
  file = paste0(params$out_path, "/", params$study,"_dmc_qval_0.05_methdiff_5.bed"),
  sep="\t",
  col.names=FALSE,
  row.names=FALSE,
  quote=FALSE
...
write.table(
  meth_diff_rgb_bed,
  file = paste0(params$out_path, "/", params$study,"_dmc_meth_diff_color.bed"),
  sep="\t",
  col.names=FALSE,
  row.names=FALSE,
  quote=FALSE
...
perc.meth.cpg$sig.group <- "NS"
perc.meth.cpg$sig.group[perc.meth.cpg$qvalue < 0.05 & abs(perc.meth.cpg$meth.diff) <= 10] <- "qvalue"
perc.meth.cpg$sig.group[perc.meth.cpg$qvalue >= 0.05 & abs(perc.meth.cpg$meth.diff) > 10] <- "meth.diff"
perc.meth.cpg$sig.group[perc.meth.cpg$qvalue < 0.05 & abs(perc.meth.cpg$meth.diff) > 10] <- "qvalue and meth.diff"
...
ggplot(
  ...
  labs(
    ...
    y = "-log10(qvalue)",
    title = paste(contrast[1], "vs", contrast[2], "total DMCs"),
    subtitle = "Thresholds: qvalue=0.05, meth.diff=10"
  ) +
  guides(
    color = guide_legend(
      override.aes = list(
